...
namespace ludaPtica
{
    public partial class Form1 : Form
    {
        ...
        private void ReadAndShowScore()
        {
                ...
                {
                    MessageBox.Show(string.Format("Čestitamo, novi rekord je {0}", label1.Text), "LudaPtica", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                StreamReader reader = new StreamReader("Score.ini");
                score = int.Parse(reader.ReadToEnd());
                reader.Close();
                if (score > int.Parse(label1.Text))
                {
                    MessageBox.Show(string.Format("Vaš rezultat je {0}", label1.Text), "LudaPtica", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void CheckForPoint()
        {
            ...
            if (!ResetPipes | start)
            {
                if (intersect1 != Rectangle.Empty | intersect2 != Rectangle.Empty)
                {
                    ...
                    {
                        points++;
                        SoundPlayer sp = new SoundPlayer(ludaPtica.Properties.Resources.point);
                        sp.Play();
                        inPipe = true;
                    }
                }
        ...
        private void CheckForCollision()
        {
            ...
            if (!ResetPipes | start)
            {
                ...
                {
                    SoundPlayer sp = new SoundPlayer(ludaPtica.Properties.Resources.collision);
                    sp.Play();
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Space:
                    step = -25;
                    pictureBox1.BackgroundImage = Resources.bird_straight;
                    break;
            }
        }
        ...
        {
            if (pictureBox1.Location.Y + pictureBox1.Height > this.ClientSize.Height + 20)
            {
                SoundPlayer sp = new SoundPlayer(ludaPtica.Properties.Resources.collision);
                sp.Play();
        ...
        private void Form1_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Space:
                    step = 7;
                    pictureBox1.BackgroundImage = Resources.bird_down;
                    break;
            }
        }
    }
